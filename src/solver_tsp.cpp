...
SolveResult solve_tsp(const Instance& inst, Rng& rng, const SolverOptions& options) {
    ...
    for (int r = 0;; ++r) {
        ...
        std::vector<int> seed;
        {
            ScopedPhaseTimer phase_timer(result.stats.phases.tsp_construction_seconds);
            seed = (r == 0) ? farthest_insertion_order(inst, all) : nearest_neighbor_order(inst, all, rng.randint(inst.N));
        }
        Tour tour;
        tour.init(inst.N);
        tour.set_tour(seed, inst);
        {
            ScopedPhaseTimer phase_timer(result.stats.phases.initial_polish_seconds);
        ...
        {
            ScopedPhaseTimer phase_timer(result.stats.phases.tsp_ils_seconds);
