...
public:
    void update(const unsigned char* data, std::size_t size) noexcept {
        ...
        while (size > 0U) {
            const std::size_t space = block_.size() - block_size_;
    ...
    std::array<unsigned char, 32> finish() noexcept {
        ...
        for (int index = 7; index >= 0; --index) {
            block_[block_size_++] = static_cast<unsigned char>(
                bit_length >> static_cast<unsigned>(index * 8));
        }
        transform(block_.data());
        block_size_ = 0U;
        ...
        for (std::size_t i = 0; i < state_.size(); ++i) {
            digest[4U * i] = static_cast<unsigned char>(state_[i] >> 24U);
            digest[4U * i + 1U] = static_cast<unsigned char>(state_[i] >> 16U);
            digest[4U * i + 2U] = static_cast<unsigned char>(state_[i] >> 8U);
            digest[4U * i + 3U] = static_cast<unsigned char>(state_[i]);
        }
        return digest;
    }

private:
    void transform(const unsigned char* input) noexcept {
        std::array<std::uint32_t, 64> words{};
        for (std::size_t i = 0; i < 16U; ++i) {
            const std::size_t offset = 4U * i;
            words[i] = (static_cast<std::uint32_t>(input[offset]) << 24U)
        ...
        for (std::size_t i = 0; i < words.size(); ++i) {
            const std::uint32_t big_sigma1 = rotate_right(e, 6U)
                                           ^ rotate_right(e, 11U)
                                           ^ rotate_right(e, 25U);
            const std::uint32_t choose = (e & f) ^ ((~e) & g);
            const std::uint32_t temp1 = h + big_sigma1 + choose
                                      + kRoundConstants[i] + words[i];
            const std::uint32_t big_sigma0 = rotate_right(a, 2U)
                                           ^ rotate_right(a, 13U)
                                           ^ rotate_right(a, 22U);
...
std::string encode_hex(const std::array<unsigned char, 32>& digest) {
