...
Each solver restart emits one typed `RestartRecord`: raw length, stable `RestartKind`, sweep, controller role, seed variant, metric-aware centroid, and mean radius. Full-TSP solves participate in the same model: their first record is `tsp-farthest-insertion` and later records are `tsp-nearest-neighbor`. The stable kind code/name table lives in `include/aldous_tsp/restart_kinds.def` and is shared by the public API, schema checks, and Python analysis.
