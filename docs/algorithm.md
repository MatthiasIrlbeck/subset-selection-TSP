...
Each solver restart emits one typed `RestartRecord`: raw length, stable `RestartKind`, sweep, controller role, seed variant, promotion stage, allocated SA iterations, metric-aware centroid, and mean radius. Full-TSP solves participate in the same model. Candidate starts are nearest-neighbor by default; an optional cubic farthest-insertion pilot is recorded as `tsp-farthest-insertion` when `--tsp-farthest-starts 1` is selected. The stable kind code/name table lives in `include/aldous_tsp/restart_kinds.def` and is shared by the public API, schema checks, and Python analysis.
