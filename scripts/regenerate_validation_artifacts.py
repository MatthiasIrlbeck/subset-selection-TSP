...
def is_native_result(document: dict[str, Any]) -> bool:
    return all(
        key in document
        for key in (
            "schema_version",
...
def validation_provenance(stage: Path) -> dict[str, Any]:
    records: dict[tuple[Any, ...], list[Path]] = {}
    ...
    if not records:
        raise RuntimeError("current validation evidence contains no native result documents")
    if len(records) != 1:
        details = "; ".join(
            f"{key!r}: {[str(path.relative_to(stage)) for path in paths]}"
            for key, paths in records.items()
        )
        raise RuntimeError(f"current validation evidence mixes source provenance: {details}")

    key = next(iter(records))
    project, schema, commit, tree, dirty, revision_source = key
    if project != project_version():
        raise RuntimeError(
            f"validation executable project version {project!r} != repository {project_version()!r}"
        )
    if schema != current_schema_version():
        raise RuntimeError(
            f"validation executable schema {schema!r} != repository {current_schema_version()!r}"
        )
    if not isinstance(commit, str) or re.fullmatch(r"[0-9a-f]{40}", commit) is None:
        raise RuntimeError(f"validation evidence has invalid source commit {commit!r}")
    if not isinstance(tree, str) or re.fullmatch(r"[0-9a-f]{40}", tree) is None:
        raise RuntimeError(f"validation evidence has invalid source tree {tree!r}")
    if dirty is not False:
        raise RuntimeError("current validation evidence must be generated from a clean source tree")
    if revision_source not in {"git", "source-archive"}:
        raise RuntimeError(f"validation evidence has invalid revision source {revision_source!r}")
    ...
    return {
        ...
        "native_result_count": sum(len(paths) for paths in records.values()),
    }


def validation_readme(summary: dict[str, Any]) -> str:
    return (
        "# Current release validation evidence\n\n"
        "These compact deterministic/smoke artifacts were generated by "
        f"solver {summary['project_version']} with result schema {summary['schema_version']}. "
        ...
        f"- solver commit: `{summary['source_commit']}`\n"
        f"- source tree: `{summary['source_tree']}`\n"
        f"- source state: clean (`{summary['revision_source']}`)\n\n"
        "Every native result in this directory repeats that exact provenance. Later "
        "release-preparation commits do not relabel these artifacts as if they had been "
        "regenerated at the release tip; regenerate the directory whenever solver behavior "
        "or validation inputs change.\n"
    )
...
def compact_summary(stage: Path, build_dir: Path | None) -> dict[str, Any]:
    provenance = validation_provenance(stage)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="aldous-current-validation-", dir=parent) as raw:
        ...
        (stage / "README.md").write_text(validation_readme(summary), encoding="utf-8")
